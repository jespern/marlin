...
pub fn ensureTlsReady(client: *std.http.Client) Error!void {
    const io = client.io;
    {
...
pub fn testUrl(allocator: std.mem.Allocator, server: *const Io.net.Server) ![:0]u8 {
    return std.fmt.allocPrintSentinel(allocator, "http://127.0.0.1:{d}/test", .{server.socket.address.getPort()}, 0);
}

pub fn serveDelayedResponse(io: Io, server: *Io.net.Server, delay_ms: i64, body: []const u8) void {
...
pub fn discardChunk(_: void, _: []const u8) bool {
    return true;
}

pub fn streamPostTask(
    comptime Ctx: type,
...
pub fn readTestRequest(io: Io, stream: Io.net.Stream) !void {
...
pub fn writeTestResponse(io: Io, stream: Io.net.Stream, keep_alive: bool) !void {
