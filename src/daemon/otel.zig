...
pub const content_attribute_cap = 32 * 1024;
...
pub fn buildEndpoint(gpa: std.mem.Allocator, raw: []const u8, traces_specific: bool) ![:0]u8 {
...
pub fn parseHeaders(gpa: std.mem.Allocator, raw: []const u8) ![][]u8 {
...
pub fn freeHeaders(gpa: std.mem.Allocator, headers: [][]u8) void {
    for (headers) |line| {
        @memset(line, 0);
        gpa.free(line);
    }
...
pub fn buildTurnContent(arena: std.mem.Allocator, store: *Store, trace: TelemetryTrace) !TurnContent {
...
pub fn buildTraceRequest(allocator: std.mem.Allocator, trace: TelemetryTrace, content: ?TurnContent) ![]u8 {
...
pub fn spanAttribute(span: std.json.Value, key: []const u8) ?std.json.Value {
