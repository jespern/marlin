...
//! crypto run in a bounded, dependency-free Node.js subprocess off dispatcher.
...
const script = @embedFile("push.cjs");

pub fn run(gpa: std.mem.Allocator, io: Io, environ: *const std.process.Environ.Map, action: []const u8, input: []const u8) ![]u8 {
    const result = try std.process.run(gpa, io, .{
        .argv = &.{ "node", "-e", script, action, input },
        .environ_map = environ,
...
pub const Worker = struct {
    ...
    pub fn init(gpa: std.mem.Allocator, io: Io, environ: *const std.process.Environ.Map) Worker {
        return .{ .gpa = gpa, .io = io, .environ = environ, .jobs = .init(gpa) };
    }
    ...
    fn work(self: *Worker) void {
        while (self.jobs.pop(self.io)) |job| {
            ...
            const result = run(self.gpa, self.io, self.environ, "deliver", job.payload) catch |err| {
