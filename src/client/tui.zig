...
/// `/screensaver daybreak cycle` or `… <hour>`: preview the sky instead of
...
pub const SetupPrompt = setup_mod.SetupPrompt;

pub const setup_provider_items = setup_mod.setup_provider_items;

const SetupReadiness = setup_mod.SetupReadiness;

const OtelCommand = commands.OtelCommand;

pub const council_done_item = "Done";
...
const PendingApproval = session_view.PendingApproval;
...
pub const OwnedCouncil = struct {
    ...
    pub fn deinit(self: *OwnedCouncil, gpa: std.mem.Allocator) void {
        gpa.free(self.name);
...
pub const TabMouseAction = enum { activate, context_menu };
...
pub const PlanItemOwned = session_view.PlanItemOwned;
pub const hasUnfinishedPlan = session_view.hasUnfinishedPlan;
pub const deinitPlan = session_view.deinitPlan;
const SessionView = session_view.SessionView;
const SearchHitOwned = search_mod.SearchHitOwned;

/// Operator-pending state for the composer's vim layer. One value per App:
/// a half-typed `d2w` belongs to the keyboard, not to any session.
pub const VimState = struct {
    /// 'd'/'c'/'y' awaiting a motion or text object; pending_obj holds
    /// 'i'/'a' awaiting the object key.
    pending_op: u8 = 0,
    pending_obj: u8 = 0,
...
pub const App = struct {
    ...
    cell_px_w: u32 = 0,
    cell_px_h: u32 = 0,
    /// Where the machine is, for daybreak's sun; resolved on first use.
    sky_observer: ?daybreak.Observer = null,
    /// `/screensaver daybreak cycle` or `… 18.5`: preview the sky instead of
    ...
    /// Composer vim state: operator-pending keys, counts, and find repeats.
    vim: VimState = .{},
    ...
    pub fn handleDaemonLine(self: *App, line: []u8) void {
        ...
        switch (msg) {
            .daemon_stopping => {
                // Deliberate shutdown must not enter the crash-reconnect path,
                // which would immediately autostart a replacement daemon.
    ...
    fn skyObserver(self: *App) daybreak.Observer {
        if (self.sky_observer) |observer| return observer;
        const observer = daybreak.observerFromSystem(self.gpa, self.io, self.environ);
        self.sky_observer = observer;
        return observer;
    }

    /// The optional last word of `/screensaver daybreak …`: an hour (0-24)
    ...
    pub fn applySkyArg(self: *App, kind: effects.Kind, arg: ?[]const u8) bool {
        ...
        if (kind != .daybreak) {
            self.setNotice("{s} takes no hour; only daybreak follows the sun", .{kind.name()});
            return false;
        }
        ...
        self.setNotice("usage: /screensaver daybreak [<hour 0-24>|cycle]", .{});
        return false;
    }

    /// The sky sun-following effects see: the real sun over the machine's
    /// place, now — unless a `/screensaver` argument or
    /// `MARLIN_DAYBREAK_HOUR` (a number, or `cycle`) picks today's hour.
    fn effectSky(self: *App) daybreak.Sky {
        const observer = self.skyObserver();
        const clock = daybreak.localClock(self.io);
        ...
        if (self.sky_override) |override| {
            ...
            if (env.get("MARLIN_DAYBREAK_HOUR")) |value| {
        ...
        return daybreak.skyFor(at, observer);
    }
    ...
    pub fn resetEffectEngine(self: *App, requested: effects.Kind) bool {
        ...
        if (kind == .daybreak and resolved.backend == .pixel) {
            ...
            self.setNotice("daybreak follows the sun over {s}{s}{d:.1}°, {d:.1}° ({t}){s}", .{
                zone,
    ...
    // Implemented in keys.zig; exposed here so call sites keep method syntax.
    pub const takeCount = keys.takeCount;
    pub const hasPending = keys.hasPending;
    pub const clearPending = keys.clearPending;
    pub const repeatForwardRange = keys.repeatForwardRange;
    pub const repeatBackwardRange = keys.repeatBackwardRange;
    pub const resolveFind = keys.resolveFind;
...
pub fn statusModel(arena: std.mem.Allocator, model: []const u8) ![]const u8 {
...
pub fn statusCwd(arena: std.mem.Allocator, cwd: []const u8, home: []const u8) ![]const u8 {
...
pub const parseOtelCommand = commands.parseOtelCommand;

pub const commandQuery = commands.commandQuery;

pub const commandSuggestions = commands.commandSuggestions;

pub const completeSuggestion = commands.completeSuggestion;
...
pub fn layoutLines(arena: std.mem.Allocator, app: *App, width: u16) !std.ArrayList(Line) {
...
pub fn validCatalogRate(rate: ?f64) ?f64 {
...
pub fn formatModelPricing(arena: std.mem.Allocator, pricing: proto.ModelPricing) ![]const u8 {
...
pub const buildReviewPrompt = commands.buildReviewPrompt;
...
pub fn nextRootTabSid(sessions: []const SessionSummary, active_sid: u64, direction: i8) ?u64 {
...
pub fn rootTabSidAtIndex(sessions: []const SessionSummary, index: usize) ?u64 {
...
pub fn layoutTabBar(arena: std.mem.Allocator, app: *const App, width: usize) !TabLayout {
...
pub fn inputPanelHeight(content_height: usize) usize {
...
pub fn planDisplayRange(items: []const PlanItemOwned, max_rows: usize) PlanDisplayRange {
...
pub fn planMarker(status: block.PlanStatus, session_state: proto.SessionState, spinner_frame: usize) PlanMarker {
...
pub const formatPlanDuration = layout_mod.formatPlanDuration;
const PlanTableWidths = layout_mod.PlanTableWidths;
pub const planTableWidths = layout_mod.planTableWidths;

pub fn planRule(arena: std.mem.Allocator, width: usize) ![]const u8 {
    var out: std.ArrayList(u8) = .empty;
    var used: usize = 0;
...
pub fn planItemTimeMs(app: *const App, item: PlanItemOwned, now_ms: i64) ?u64 {
...
pub fn drawUiAnimation(app: *const App, win: vaxis.Window) void {
...
pub const TransportVerdicts = struct {
...
pub const tabMouseAction = keys.tabMouseAction;

pub const handleMouse = keys.handleMouse;
...
pub const isEnterKey = keys.isEnterKey;

pub const isNewlineKey = keys.isNewlineKey;

pub const EditCommand = keys.EditCommand;

pub const editCommand = keys.editCommand;

pub const isCommandInput = commands.isCommandInput;

pub const isPreviousInputRowKey = keys.isPreviousInputRowKey;

pub const isNextInputRowKey = keys.isNextInputRowKey;

pub const isNewSessionKey = keys.isNewSessionKey;

pub const isArchiveCurrentKey = keys.isArchiveCurrentKey;

pub const isArchivePickerKey = keys.isArchivePickerKey;

const applyEditCommand = keys.applyEditCommand;

pub const isPlanToggleKey = keys.isPlanToggleKey;
...
const isVoiceKey = keys.isVoiceKey;
...
pub const handleKey = keys.handleKey;
