...
pub fn removeSectionText(gpa: std.mem.Allocator, current: []const u8, header: []const u8) ![]u8 {
    var offset: usize = 0;
    while (offset < current.len) {
        const line_end = std.mem.indexOfScalarPos(u8, current, offset, '\n') orelse current.len;
        const next = if (line_end < current.len) line_end + 1 else line_end;
...
pub fn removeCouncilText(gpa: std.mem.Allocator, current: []const u8, name: []const u8) ![]u8 {
    try validateToolName(name);
    var offset: usize = 0;
    while (offset < current.len) {
        const line_end = std.mem.indexOfScalarPos(u8, current, offset, '\n') orelse current.len;
...
pub fn removeMcpServerText(gpa: std.mem.Allocator, current: []const u8, name: []const u8) ![]u8 {
    try validateToolName(name);
    var offset: usize = 0;
    while (offset < current.len) {
        const line_end = std.mem.indexOfScalarPos(u8, current, offset, '\n') orelse current.len;
...
pub fn applyDocument(cfg: *Config, doc: toml.Document) void {
...
pub fn validate(gpa: std.mem.Allocator, cfg: Config) !void {
